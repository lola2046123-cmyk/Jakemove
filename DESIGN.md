# SkillDock 设计规范（仓库活跃版）

本文件为 **SkillDock** 前端实现的唯一权威设计说明。写 UI 或调样式时优先阅读 **§0**，需要颜色语义与排版原则时继续阅读 **§1–§9**（Anthropic Claude 设计叙事）。

`claude/DESIGN.md` 由 `npx getdesign@latest add claude` 生成；若脚手架更新了 Anthropic 正文，请将 `claude/DESIGN.md` 中自「# Design System Inspired by Claude」起的章节同步进本文件对应位置（保留本文件开头的 §0）。详见 `claude/README.md`。

---

## 0. SkillDock 与代码映射

- **实现入口**：`src/theme/tokens.ts`（`lightTokens` / `darkTokens`）、`src/context/ThemeContext.tsx`。
- **页面底色**：羊皮纸 `#f5f4ed` → `bgPage`（亮色）；暗色页背景 `#141413` → `bgPage`（暗色）。
- **布局**：主容器约 `1280px`；发现页为「弹性主栏 + 固定 280px 侧栏」两列栅格。
- **焦点环**：`focusBlue` `#3898ec`（系统中唯一偏冷的强调色，仅用于 focus）。
- **i18n**：默认 **简体中文 `zh-CN`**（`src/locales/zh.json`）；切换逻辑见 `src/i18n/index.tsx`。中文界面下技术术语保持英文：**Skill**、**Prompt**、**Manifest**、**Cursor**、**Claude Code** 等按产品约定不译。

### 0.1 设计色名与代码 token 对照（亮色）

| Anthropic 命名 | 代码 token | 色值 |
|----------------|------------|------|
| Parchment | `bgPage` | `#f5f4ed` |
| Ivory | `bgCard` | `#faf9f5` |
| Pure White | `bgInput` / `bgModal` | `#ffffff` |
| Warm Sand | `bgSand` | `#e8e6dc` |
| Terracotta Brand | `brandTerracotta` | `#c96442` |
| Border Cream | `borderLight` | `#f0eee6` |
| Border Warm | `borderMedium` | `#e8e6dc` |
| Anthropic Near Black | `textPrimary` | `#141413` |
| Olive Gray | `textSecondary` | `#5e5d59` |
| Stone Gray | `textTertiary` | `#87867f` |
| Dark Surface | `borderDark` / `bgDarkSurface` | `#30302e` |
